...
The system includes:
- Classical **single‑task baselines** (KNN, Random Forest, XGBoost)
...
- `GET /health` – Service health check and basic artefact status (model loaded, feature count)
- `POST /predict` – Single-record inference via JSON payload (attack + device), includes latency and top-k probabilities
- `POST /api/predict-file` – Batch inference via CSV upload (raw CIC packet CSV); returns distributions, timing breakdown, throughput and capped per-row predictions
- `GET /api/schema` – Returns model feature schema (feature names + expected count)
- `GET /api/labels` – Returns label map summary (counts + sample mappings)
- `GET /api/info` – Model metadata and (if available) stored evaluation summary
...
│   └── src/
|   |   └── components/
├── notebooks/            # Baselines + EDA + Model Training
├── deployment/
